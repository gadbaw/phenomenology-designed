...
cd "$BUILD"
for g in gen_explorer gen_browser gen_arcs gen_viz gen_patterns gen_glossary; do
  "$PY" "$g.py" --share >/dev/null
done
cp index.html units.html arcs.html viz.html patterns.html guide.html "$REPO"/ 2>/dev/null || true
